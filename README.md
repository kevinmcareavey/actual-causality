...
The values of these variables are defined in a causal model with on one additional exogenous variables `U_A` and where the additioanl structural equations are defined as `A = U_A` and `V = pass` if `O == A` or `O == unknown` or `A == unknown`, otherwise `V = fail`.
